...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_equipment']) && $_SESSION['role'] == 'admin') {
    ...
    $add_stmt = $conn->prepare("INSERT INTO equipment (name, category, location, status, notes, created_by) VALUES (?, ?, ?, ?, ?, ?)");
    if ($add_stmt) {
        $add_stmt->bind_param("sssssi", $name, $category, $location, $status, $notes, $created_by);
        if ($add_stmt->execute()) {
            $_SESSION['success'] = "New equipment added successfully!";
        } else {
            $_SESSION['error'] = "Error adding equipment: " . $add_stmt->error;
        }
        $add_stmt->close();
    }
    header("Location: equipment_monitoring.php");
...
// Categories & Locations dropdown queries
...
<div class="gym-main-container">
  ...
  <div class="gym-page-header">
    <div>
      <h1 class="gym-page-title" style="display: flex; align-items: center; gap: 10px;">
        ...
        Equipment & Facility Status Monitoring
      </h1>
      <p class="gym-page-subtitle">Track operational fitness equipment, reported breakdowns, maintenance logs, and facility areas.</p>
    </div>
    <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
      <a href="maintenance_report.php" class="gym-btn gym-btn-yellow">
        <i data-lucide="file-text"></i> Maintenance Reports
      </a>
      <?php if ($role == 'admin'): ?>
        <button type="button" onclick="openAddEquipmentModal()" class="gym-btn gym-btn-outline" style="border-color: var(--accent); color: var(--accent);">
  ...
  <div class="gym-stats-grid">
    ...
    <div class="gym-stat-card">
      <div>
        ...
        <div style="font-size: 0.75rem; color: var(--text-dim); margin-top: 4px;">Unusable machines</div>
      </div>
    ...
    <div class="gym-stat-card">
      <div>
        <div class="gym-stat-label">Monitored Facilities</div>
        <div class="gym-stat-number" style="color: #3b82f6;"><?php echo number_format($total_facilities); ?></div>
        <div style="font-size: 0.75rem; color: var(--text-dim); margin-top: 4px;">Gym areas & zones</div>
      </div>
      ...
      <div class="gym-table-wrapper" style="margin-bottom: 0;">
        <table class="gym-table">
          ...
          <tbody>
                    ...
                    <td>
                      <div style="display: flex; gap: 6px; align-items: center; justify-content: center;">
                        <button type="button" onclick="openUpdateStatusModal(<?php echo htmlspecialchars(json_encode($equipment)); ?>)" class="gym-btn gym-btn-outline" style="min-height: 32px !important; padding: 4px 10px !important; font-size: 0.78rem !important; color: #60a5fa !important; border-color: rgba(96, 165, 250, 0.3) !important;" title="Update Status">
                          <i data-lucide="edit" style="width: 14px; height: 14px;"></i> Status
                        </button>
                        <?php if ($role == 'admin'): ?>
            ...
            <?php else: ?>
              <tr>
                <td colspan="<?php echo ($role == 'admin' || $role == 'trainer') ? 6 : 5; ?>" style="text-align: center; color: var(--text-dim); padding: 3rem 1rem;">
      ...
      <div class="gym-table-wrapper" style="margin-bottom: 0;">
        <table class="gym-table">
          ...
          <tbody>
            ...
            <?php else: ?>
              <tr>
                <td colspan="<?php echo ($role == 'admin' || $role == 'trainer') ? 5 : 4; ?>" style="text-align: center; color: var(--text-dim); padding: 3rem 1rem;">
  ...
  <!-- Add Equipment Modal -->
  <?php if ($role == 'admin'): ?>
  <div id="addEquipmentModal" class="modal" style="display: none; position: fixed; inset: 0; z-index: 9999; background: rgba(0,0,0,0.7); align-items: center; justify-content: center;">
    <div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-md); width: 100%; max-width: 480px; padding: 24px; margin: auto;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h3 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.2rem; color: var(--accent); margin: 0;">Add New Gym Equipment</h3>
        <button type="button" onclick="closeAddEquipmentModal()" style="background: transparent; border: none; color: var(--text-dim); cursor: pointer; font-size: 1.2rem;">
          <i data-lucide="x"></i>
        </button>
      </div>
      ...
      <form method="POST" style="display: flex; flex-direction: column; gap: 14px;">
        ...
        <div>
          <label class="gym-form-label">Equipment Name *</label>
          <input type="text" name="name" required class="gym-form-control" placeholder="e.g. Treadmill #4">
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
          <div>
            <label class="gym-form-label">Category *</label>
            <input type="text" name="category" required class="gym-form-control" placeholder="Cardio / Strength">
          </div>
          <div>
            <label class="gym-form-label">Location Zone *</label>
            <input type="text" name="location" required class="gym-form-control" placeholder="Main Floor">
          </div>
        </div>

        <div>
          <label class="gym-form-label">Initial Status *</label>
          <select name="status" required class="gym-form-control">
        ...
        <div>
          <label class="gym-form-label">Notes / Remarks</label>
          <textarea name="notes" rows="3" class="gym-form-control" placeholder="Optional notes..."></textarea>
        </div>

        <div style="display: flex; gap: 10px; margin-top: 10px;">
          <button type="button" onclick="closeAddEquipmentModal()" class="gym-btn gym-btn-outline" style="flex: 1;">Cancel</button>
          <button type="submit" name="add_equipment" class="gym-btn gym-btn-yellow" style="flex: 1;">Add Equipment</button>
        </div>
      </form>
    </div>
  </div>
    ...
    <div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-md); width: 100%; max-width: 480px; padding: 24px; margin: auto;">
      ...
      <form method="POST" style="display: flex; flex-direction: column; gap: 14px;">
        ...
        <div>
          ...
          <textarea name="note" id="status_note" rows="3" class="gym-form-control" placeholder="Describe the issue, fix, or maintenance work done..."></textarea>
        </div>
